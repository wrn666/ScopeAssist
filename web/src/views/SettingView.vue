    ...
    <div class="setting-container layout-container">
      ...
      <div class="setting" v-if="(state.windowWidth <= 520 || state.section === 'base') && userStore.isSuperAdmin">
        ...
        <div class="section">
          ...
          <div class="card card-select">
            <span class="label">{{ items?.vl_model.des }}</span>
            <ModelSelectorComponent
              @select-model="handleVLModelSelect"
              :model_name="vlModelName"
              :model_provider="vlModelProvider"
            />
          </div>
          <div class="card card-select">
...
const vlModelProvider = computed(() => {
  const vlModel = configStore.config?.vl_model
  if (!vlModel) return ''
  return vlModel.split('/')[0]
})

const vlModelName = computed(() => {
  const vlModel = configStore.config?.vl_model
  if (!vlModel) return ''
  const parts = vlModel.split('/')
  return parts.slice(1).join('/')
})

const handleVLModelSelect = ({ provider, name }) => {
  configStore.setConfigValue('vl_model', `${provider}/${name}`)
}
