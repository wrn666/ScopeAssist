  ...
  <div class="file-table-container">
    ...
    <a-table
      ...
      <template #bodyCell="{ column, text, record }">
        <div v-if="column.key === 'filename'" class="filename-cell">
          <div v-if="editingFileId !== record.file_id" class="filename-display" @dblclick.stop="startEditFilename(record)">
          ...
          <div v-else class="filename-edit">
            <a-input
              v-model:value="editingFilename"
              size="small"
              @pressEnter="saveFilename(record)"
              @blur="handleBlur(record)"
              @keydown.esc="cancelEditFilename"
              :ref="(el) => filenameInputRef = el"
              style="flex: 1;"
            />
            <a-button type="text" size="small" @click.stop="saveFilename(record)" :loading="updatingFilename" style="margin-left: 4px;" title="保存">
              ✓
            </a-button>
            <a-button type="text" size="small" @click.stop="cancelEditFilename" style="margin-left: 2px;" title="取消">
              ✕
            </a-button>
          </div>
        </div>
        ...
        <div v-else-if="column.key === 'action'" style="display: flex; gap: 4px;">
          <a-button class="download-btn" type="text"
            ...
            title="下载"
            />
          <a-button class="edit-btn" type="text"
            @click="startEditFilename(record)"
...
const updatingFilename = ref(false);
...
const columnsCompact = [
  ...
  { title: '', key: 'action', dataIndex: 'file_id', width: 120, align: 'center' }
];
...
// 文件名编辑功能
const startEditFilename = async (record) => {
  if (lock.value || record.status === 'processing' || record.status === 'waiting') {
    return; // 不允许编辑正在处理或等待中的文件
  }
  editingFileId.value = record.file_id;
  editingFilename.value = record.filename;
  await nextTick();
...
// 处理输入框失焦事件，延迟保存以避免与点击按钮冲突
let blurTimer = null;
const handleBlur = (record) => {
  // 延迟执行，以便点击保存/取消按钮能正常触发
  blurTimer = setTimeout(() => {
    if (editingFileId.value === record.file_id) {
      saveFilename(record);
    }
  }, 200);
};

const saveFilename = async (record) => {
  // 清除blur定时器
  if (blurTimer) {
  ...
  if (editingFilename.value.trim() === record.filename) {
    // 文件名没有变化，直接取消编辑
    cancelEditFilename();
    return;
  }
  ...
  updatingFilename.value = true;
  try {
    await documentApi.updateDocumentFilename(dbId, record.file_id, editingFilename.value.trim());
    ...
    message.error(error.message || '更新文件名失败，请稍后重试');
    // 失败时不取消编辑，让用户可以重试
  } finally {
    updatingFilename.value = false;
  }
};
</script>
...
.my-table-compact .edit-btn {
  color: var(--gray-500);
}

.my-table-compact .edit-btn:hover {
  color: var(--main-color);
}
