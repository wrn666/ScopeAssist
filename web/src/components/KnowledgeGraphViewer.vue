...
import { codehubGraphApi } from '@/apis/graph_api'
...
const props = defineProps({
  ...
  kbType: {
    type: String,
    default: 'lightrag' // 'lightrag' 或 'codehub'
  }
})
...
const loadAvailableDatabases = async () => {
  ...
  // CodeHub 知识库不需要加载数据库列表（使用传入的数据库ID）
  if (props.kbType === 'codehub' && props.initialDatabaseId) {
    selectedDatabase.value = props.initialDatabaseId
    await loadGraphLabels(selectedDatabase.value)
    return
  }
...
const loadGraphData = async () => {
  ...
  try {
    ...
    // 根据知识库类型选择不同的 API
    if (props.kbType === 'codehub') {
      // CodeHub 知识库
      graphResponse = await codehubGraphApi.getSubgraph({
        db_id: selectedDatabase.value,
        max_nodes: searchParams.max_nodes
      })
      
      // CodeHub 暂时没有统计信息 API，使用图数据中的统计
      statsResponse = {
        success: true,
        data: {
          entity_types: [],
          total_nodes: graphResponse.data?.total_nodes || 0,
          total_edges: graphResponse.data?.total_edges || 0
        }
      }
    } else {
      // LightRAG 知识库
...
const expandNode = async (nodeId) => {
  ...
  try {
    ...
    if (props.kbType === 'codehub') {
      // CodeHub 暂时不支持展开节点
      message.info('CodeHub 知识库暂不支持展开节点功能')
      expanding.value = false
      return
    } else {
      response = await lightragApi.getSubgraph({
        db_id: selectedDatabase.value,
        node_label: nodeId,
        max_depth: 1,
        max_nodes: 50
