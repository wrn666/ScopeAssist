<template>
  ...
  <!-- Unstructured 可视化弹窗 -->
  <UnstructuredVisualizerModal v-model:visible="visualizerModalVisible" />
</template>
...
const ocrHealthStatus = ref({
  ...
  unstructured: { status: 'healthy', message: 'Unstructured 服务就绪' }
});
...
const enableOcrOptions = computed(() => [
  ...
  {
    value: 'unstructured',
    label: getUnstructuredLabel(),
    title: 'Unstructured (智能文档解析)',
    disabled: ocrHealthStatus.value.unstructured.status === 'unavailable' || ocrHealthStatus.value.unstructured.status === 'error'
  },
  {
    value: 'mineru_ocr',
  ...
  {
    value: 'mineru_cloud',
    label: getMinerUCloudLabel(),
    title: 'MinerU Cloud (官方云服务)',
    disabled: ocrHealthStatus.value.mineru_cloud.status === 'unavailable' || ocrHealthStatus.value.mineru_cloud.status === 'error'
  },
  {
    value: 'paddlex_ocr',
...
const selectedOcrStatus = computed(() => {
  switch (chunkParams.value.enable_ocr) {
    case 'unstructured':
      return ocrHealthStatus.value.unstructured.status;
    case 'mineru_ocr':
      return ocrHealthStatus.value.mineru_ocr.status;
    case 'mineru_cloud':
      return ocrHealthStatus.value.mineru_cloud.status;
    case 'paddlex_ocr':
...
const selectedOcrMessage = computed(() => {
  switch (chunkParams.value.enable_ocr) {
    case 'unstructured':
      return ocrHealthStatus.value.unstructured.message;
    case 'mineru_ocr':
      return ocrHealthStatus.value.mineru_ocr.message;
    case 'mineru_cloud':
      return ocrHealthStatus.value.mineru_cloud.message;
    case 'paddlex_ocr':
...
// OCR选项标签生成函数
const getUnstructuredLabel = () => {
  const status = ocrHealthStatus.value.unstructured.status;
  const statusIcons = {
    'healthy': '✅',
    'unavailable': '❌',
    'error': '⚠️',
  ...
  return `${statusIcons[status] || '✅'} Unstructured (智能解析)`;
};
...
const getMinerUCloudLabel = () => {
  const status = ocrHealthStatus.value.mineru_cloud.status;
  const statusIcons = {
    'healthy': '✅',
    'unavailable': '❌',
    'unhealthy': '⚠️',
  ...
  return `${statusIcons[status] || '❓'} MinerU Cloud`;
};
...
const checkOcrHealth = async () => {
  ...
  try {
    ...
    // 合并健康状态，保留 unstructured 的状态（不需要检查）
