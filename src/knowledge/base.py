...
class KnowledgeBase(ABC):
    ...
    def _check_visualization_exists(self, file_id: str) -> bool:
        """
        检查文件是否有可视化数据（不加载数据，只检查文件存在）

        Args:
            file_id: 文件ID

        Returns:
            True 如果可视化文件存在，否则 False
        """
        ...
        return os.path.exists(visualization_path)

    def _load_visualization_data(self, file_id: str) -> dict | None:
        """
        加载文件的可视化数据（如果存在）
        ...
        Returns:
            可视化数据字典，如果不存在则返回None
        """
        logger.debug(f"Attempting to load visualization for file_id={file_id}")
        
        if file_id not in self.files_meta:
            logger.warning(f"File {file_id} not found in files_meta. Available files: {list(self.files_meta.keys())[:5]}...")
            return None
        ...
        if not file_path:
            logger.warning(f"No file path found for {file_id}")
            return None

        # 尝试加载可视化数据
        visualization_path = file_path + ".visualization.json"
        logger.debug(f"Checking visualization path: {visualization_path}")
        
        if os.path.exists(visualization_path):
            try:
                with open(visualization_path, encoding="utf-8") as f:
                    visualization_data = json.load(f)
                logger.info(f"Successfully loaded visualization data for file {file_id} from {visualization_path}")
                return visualization_data
            except Exception as e:
                logger.error(f"Failed to load visualization data for file {file_id}: {e}")
        else:
            logger.warning(f"Visualization file does not exist: {visualization_path}")
