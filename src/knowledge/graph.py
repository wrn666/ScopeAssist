...
class GraphDatabase:
    ...
    def start(self):
        ...
        try:
            # Neo4j 5.x: driver 连接时不指定数据库，数据库切换通过 session 的 database 参数实现
            ...
            logger.error(f"Failed to connect to Neo4j: {e}, {uri}, {username}, {password}")
    ...
    def get_sample_nodes(self, kgdb_name="neo4j", num=50):
        """获取指定知识库的 num 个节点信息，优先返回连通的节点子图"""
        assert self.driver is not None, "Database is not connected"
        self.use_database(kgdb_name)

        def query(tx, num, kgdb_id):
            """Note: 使用连通性查询获取集中的节点子图"""
            # 首先尝试获取一个连通的子图
            query_str = """
                // 获取高度数节点作为种子节点
                MATCH (seed:Entity)
                WHERE seed.kgdb_id = $kgdb_id
                ...
                WHERE neighbor.kgdb_id = $kgdb_id
                ...
                WHERE second_hop <> s AND second_hop.kgdb_id = $kgdb_id
                // 增加二跳节点的数量
            ...
            try:
                results = tx.run(query_str, num=int(num), kgdb_id=kgdb_id)
                ...
                if len(formatted_results["nodes"]) < num:
                    ...
                    supplement_query = """
                    MATCH (n:Entity)
                    WHERE n.kgdb_id = $kgdb_id AND NOT elementId(n) IN $existing_ids
                    RETURN {id: elementId(n), name: n.name} AS node
                    LIMIT $count
                    """

                    supplement_results = tx.run(supplement_query, kgdb_id=kgdb_id, existing_ids=list(node_ids), count=remaining_count)
                ...
                WHERE n.kgdb_id = $kgdb_id AND m.kgdb_id = $kgdb_id AND elementId(n) < elementId(m)
                RETURN
                    {id: elementId(n), name: n.name} AS h,
                ...
                LIMIT $num
                """
                results = tx.run(fallback_query, kgdb_id=kgdb_id, num=int(num))
                formatted_results = {"nodes": [], "edges": []}
                node_ids = set()
        ...
        with self.driver.session(database="neo4j") as session:
            results = session.execute_read(query, num, kgdb_name)
            return results

    def create_graph_database(self, kgdb_name):
        """记录知识库ID（Neo4j Community Edition 不支持多数据库，使用节点属性区分）"""
        assert self.driver is not None, "Database is not connected"
        # Neo4j Community Edition 不支持多数据库，只记录知识库ID用于后续节点属性过滤
        logger.info(f"使用知识库ID '{kgdb_name}' (Neo4j Community Edition 不支持多数据库)")
        return kgdb_name

    def use_database(self, kgdb_name="neo4j"):
        """记录当前知识库ID（Neo4j Community Edition 不支持多数据库，使用节点属性区分）"""
        # Neo4j Community Edition 不支持多数据库，只记录知识库ID用于节点属性过滤
        if kgdb_name != self.kgdb_name:
            logger.debug(f"Switching knowledge base context from '{self.kgdb_name}' to '{kgdb_name}'")
    ...
    async def txt_add_vector_entity(self, triples, kgdb_name="neo4j"):
        ...
        def _create_graph(tx, data, kgdb_id):
            """添加一个三元组，使用 kgdb_id 属性区分不同知识库"""
            for entry in data:
                tx.run(
                    """
                MERGE (h:Entity:Upload {name: $h})
                SET h.kgdb_id = $kgdb_id
                MERGE (t:Entity:Upload {name: $t})
                SET t.kgdb_id = $kgdb_id
        ...
        def _get_nodes_without_embedding(tx, entity_names, kgdb_id):
            """获取没有embedding的节点列表"""
            ...
            result = tx.run(
                f"""
            MATCH (n:Entity)
            WHERE n.name IN [{param_placeholders}] AND n.kgdb_id = $kgdb_id AND n.embedding IS NULL
            RETURN n.name AS name
            """,
                {**params, "kgdb_id": kgdb_id},
            )

            return [record["name"] for record in result]

        def _batch_set_embeddings(tx, entity_embedding_pairs, kgdb_id):
            """批量设置实体的嵌入向量"""
            for entity_name, embedding in entity_embedding_pairs:
                tx.run(
                    """
                MATCH (e:Entity {name: $name})
                WHERE e.kgdb_id = $kgdb_id
        ...
        with self.driver.session(database="neo4j") as session:
            logger.info(f"Adding entity to knowledge base {kgdb_name}")
            session.execute_write(_create_graph, triples, kgdb_name)
            ...
            # 筛选出没有embedding的节点
            nodes_without_embedding = session.execute_read(_get_nodes_without_embedding, all_entities, kgdb_name)
            ...
            for i in range(0, total_entities, max_batch_size):
                ...
                # 批量写入数据库
                session.execute_write(_batch_set_embeddings, entity_embedding_pairs, kgdb_name)
    ...
    def delete_entity(self, entity_name=None, kgdb_name="neo4j"):
        """删除知识库中的指定实体三元组, 参数entity_name为空则删除该知识库的全部实体"""
        assert self.driver is not None, "Database is not connected"
        self.use_database(kgdb_name)
        with self.driver.session(database="neo4j") as session:
            if entity_name:
                session.execute_write(self._delete_specific_entity, entity_name, kgdb_name)
            else:
                session.execute_write(self._delete_all_entities, kgdb_name)

    def _delete_specific_entity(self, tx, entity_name, kgdb_id):
        query = """
        MATCH (n {name: $entity_name})
        WHERE n.kgdb_id = $kgdb_id
        DETACH DELETE n
        """
        tx.run(query, entity_name=entity_name, kgdb_id=kgdb_id)

    def _delete_all_entities(self, tx, kgdb_id):
        query = """
        MATCH (n)
        WHERE n.kgdb_id = $kgdb_id
        DETACH DELETE n
        """
        tx.run(query, kgdb_id=kgdb_id)
    ...
    def _query_with_fuzzy_match(self, keyword, kgdb_name="neo4j"):
        ...
        def query_fuzzy_match(tx, keyword, kgdb_id):
            result = tx.run(
                """
            MATCH (n:Entity)
            WHERE n.kgdb_id = $kgdb_id AND toLower(n.name) CONTAINS toLower($keyword)
        ...
        with self.driver.session(database="neo4j") as session:
            return session.execute_read(query_fuzzy_match, keyword, kgdb_name)
    ...
    def _query_with_vector_sim(self, keyword, kgdb_name="neo4j", threshold=0.9):
        ...
        def query_by_vector(tx, text, threshold, kgdb_id):
            # 首先检查索引是否存在
            ...
            WHERE similarEntity.kgdb_id = $kgdb_id
        ...
        with self.driver.session(database="neo4j") as session:
            results = session.execute_read(query_by_vector, keyword, threshold=threshold, kgdb_id=kgdb_name)
            return results
    ...
    def _query_specific_entity(self, entity_name, kgdb_name="neo4j", hops=2, limit=100):
        ...
        def query(tx, entity_name, hops, limit, kgdb_id):
            try:
                ...
                LIMIT $limit
                """
                results = tx.run(query_str, entity_name=entity_name, limit=limit, kgdb_id=kgdb_id)
        ...
        try:
            with self.driver.session(database="neo4j") as session:
                return session.execute_read(query, entity_name, hops, limit, kgdb_name)
    ...
    def set_embedding(self, tx, entity_name, embedding, kgdb_id):
        tx.run(
            """
        MATCH (e:Entity {name: $name})
        WHERE e.kgdb_id = $kgdb_id
    ...
    def get_graph_info(self, graph_name="neo4j"):
        ...
        def query(tx, kgdb_id):
            # 只统计包含Entity标签的节点，且属于指定知识库
            entity_count = tx.run(
                "MATCH (n:Entity) WHERE n.kgdb_id = $kgdb_id RETURN count(n) AS count", 
                kgdb_id=kgdb_id
            ).single()["count"]
            # 只统计包含RELATION标签的关系，且连接的节点都属于指定知识库
            relationship_count = tx.run(
                "MATCH (n:Entity)-[r:RELATION]->(m:Entity) WHERE n.kgdb_id = $kgdb_id AND m.kgdb_id = $kgdb_id RETURN count(r) AS count",
                kgdb_id=kgdb_id
            ).single()["count"]
            triples_count = tx.run(
                "MATCH (n:Entity)-[r:RELATION]->(m:Entity) WHERE n.kgdb_id = $kgdb_id AND m.kgdb_id = $kgdb_id RETURN count(n) AS count",
                kgdb_id=kgdb_id
            ).single()["count"]
        ...
        try:
            if self.is_running():
                # 获取数据库信息
                with self.driver.session(database="neo4j") as session:
                    graph_info = session.execute_read(query, graph_name)
    ...
    def query_nodes_without_embedding(self, kgdb_name="neo4j"):
        ...
        def query(tx, kgdb_id):
            result = tx.run("""
            MATCH (n:Entity)
            WHERE n.kgdb_id = $kgdb_id AND n.embedding IS NULL
            RETURN n.name AS name
            """, kgdb_id=kgdb_id)
            return [record["name"] for record in result]

        with self.driver.session(database="neo4j") as session:
            return session.execute_read(query, kgdb_name)
        ...
        with self.driver.session(database="neo4j") as session:
            for node_name in node_names:
                try:
                    ...
                    session.execute_write(self.set_embedding, node_name, embedding, kgdb_name)
                    count += 1
                except Exception as e:
