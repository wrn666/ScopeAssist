...
class KnowledgeBaseManager:
    ...
    def __init__(self, work_dir: str):
        ...
        # 确保默认CodeHub知识库存在
        self._ensure_default_codehub_database()
    ...
    def _ensure_default_codehub_database(self):
        """确保默认CodeHub知识库存在"""
        try:
            # 检查是否已存在默认CodeHub知识库
            default_codehub_name = "CodeHub"
            default_codehub_id = None
            
            # 查找现有的CodeHub知识库
            for db_id, db_meta in self.global_databases_meta.items():
                if db_meta.get("kb_type", "").lower() == "codehub" and db_meta.get("name") == default_codehub_name:
                    default_codehub_id = db_id
                    break
            
            # 如果不存在，创建默认CodeHub知识库
            if default_codehub_id is None:
                logger.info("Creating default CodeHub knowledge base")
                kb_instance = self._get_or_create_kb_instance("codehub")
                
                # 使用默认嵌入模型
                from src import config
                default_embed_model = list(config.embed_model_names.keys())[0] if config.embed_model_names else None
                embed_info = config.embed_model_names.get(default_embed_model) if default_embed_model else None
                
                db_info = kb_instance.create_database(
                    database_name=default_codehub_name,
                    description="默认代码仓库知识库，自动索引git clone的仓库",
                    embed_info=embed_info,
                )
                default_codehub_id = db_info["db_id"]
                
                # 保存到全局元数据
                self.global_databases_meta[default_codehub_id] = {
                    "name": default_codehub_name,
                    "description": "默认代码仓库知识库，自动索引git clone的仓库",
                    "kb_type": "codehub",
                    "created_at": datetime.now().isoformat(),
                    "additional_params": {},
                    "is_default": True,  # 标记为默认知识库
                }
                self._save_global_metadata()
                logger.info(f"Created default CodeHub knowledge base: {default_codehub_id}")
            else:
                # 确保标记为默认知识库
                if not self.global_databases_meta[default_codehub_id].get("is_default"):
                    self.global_databases_meta[default_codehub_id]["is_default"] = True
                    self._save_global_metadata()
                logger.debug(f"Default CodeHub knowledge base already exists: {default_codehub_id}")
                
        except Exception as e:
            logger.error(f"Failed to ensure default CodeHub database: {e}\n{traceback.format_exc()}")

    def get_default_codehub_database_id(self) -> str | None:
        """获取默认CodeHub知识库ID"""
        for db_id, db_meta in self.global_databases_meta.items():
            if db_meta.get("kb_type", "").lower() == "codehub" and db_meta.get("is_default"):
                return db_id
        # 如果没有标记为默认的，返回第一个CodeHub知识库
        for db_id, db_meta in self.global_databases_meta.items():
            if db_meta.get("kb_type", "").lower() == "codehub":
                return db_id
        return None
    ...
    async def update_file_name(self, db_id: str, file_id: str, new_filename: str) -> dict:
        """更新文件名"""
        kb_instance = self._get_kb_for_database(db_id)
        return await kb_instance.update_file_name(db_id, file_id, new_filename)

    async def update_chunk_content(self, db_id: str, file_id: str, chunk_id: str, new_content: str) -> dict:
        """更新chunk内容"""
        kb_instance = self._get_kb_for_database(db_id)
        return await kb_instance.update_chunk_content(db_id, file_id, chunk_id, new_content)

    def get_file_visualization(self, db_id: str, file_id: str) -> dict | None:
        """获取文件的可视化数据"""
        kb_instance = self._get_kb_for_database(db_id)
        return kb_instance._load_visualization_data(file_id)
    ...
    def is_codehub_database(self, db_id: str) -> bool:
        """
        检查数据库是否是 CodeHub 类型
        ...
        if db_id not in self.global_databases_meta:
            return False
        kb_type = self.global_databases_meta[db_id].get("kb_type", "").lower()
        return kb_type == "codehub"
