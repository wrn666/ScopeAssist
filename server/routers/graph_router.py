...
@graph.get("/codehub/subgraph")
async def get_codehub_subgraph(
    db_id: str = Query(..., description="数据库ID"),
    max_nodes: int = Query(200, description="最大节点数", ge=1, le=1000),
    current_user: User = Depends(get_admin_user),
):
    """
    获取 CodeHub 知识库的知识图谱子图

    Args:
        db_id: CodeHub 数据库实例ID
        max_nodes: 返回的最大节点数量

    Returns:
        包含节点和边的知识图谱数据（格式与 LightRAG 兼容）
    """
    try:
        logger.info(f"获取 CodeHub 子图数据 - db_id: {db_id}, max_nodes: {max_nodes}")

        # 检查是否是 CodeHub 数据库
        if not knowledge_base.is_codehub_database(db_id):
            raise HTTPException(
                status_code=400, detail=f"数据库 {db_id} 不是 CodeHub 类型，图谱功能仅支持 CodeHub 知识库"
            )

        # 获取 CodeHub 图构建器
        from src.knowledge.codehub_graph import CodeHubGraphBuilder
        graph_builder = CodeHubGraphBuilder()
        kgdb_name = graph_builder._get_kgdb_name(db_id)

        if not graph_base.is_running():
            graph_base.start()

        graph_base.use_database(kgdb_name)

        # 查询所有节点和边
        with graph_base.driver.session(database="neo4j") as session:
            # 获取节点
            node_query = """
            MATCH (n:Entity)
            WHERE n.kgdb_id = $kgdb_id
            RETURN elementId(n) as id, n.name as name, properties(n) as properties
            LIMIT $max_nodes
            """
            node_results = session.run(node_query, kgdb_id=kgdb_name, max_nodes=max_nodes)
            ...
            if node_ids:
                ...
                WHERE n.kgdb_id = $kgdb_id AND m.kgdb_id = $kgdb_id
                  AND elementId(n) IN $node_ids AND elementId(m) IN $node_ids
                RETURN elementId(n) as source, elementId(m) as target, 
                       type(r) as type, properties(r) as properties
                LIMIT $max_edges
                """
                max_edges = max_nodes * 2  # 边的数量通常是节点的2倍左右
                edge_results = session.run(edge_query, kgdb_id=kgdb_name, node_ids=list(node_ids), max_edges=max_edges)
                ...
                logger.info(f"成功获取 CodeHub 子图 - 节点数: {len(nodes)}, 边数: {len(edges)}")
                return result
            else:
                return {
                    "success": True,
        ...
        logger.error(f"获取 CodeHub 子图数据失败: {e}")
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"获取 CodeHub 子图数据失败: {str(e)}")
