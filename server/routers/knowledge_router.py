...
@knowledge.get("/databases/{db_id}/documents/{doc_id}/visualization")
async def get_document_visualization(db_id: str, doc_id: str, current_user: User = Depends(get_required_user)):
    """获取文档的可视化数据（Unstructured）"""
    logger.info(f"GET document {doc_id} visualization in {db_id}")
    
    try:
        viz_data = knowledge_base.get_file_visualization(db_id, doc_id)
        logger.info(f"Visualization data found: {viz_data is not None}, db_id={db_id}, doc_id={doc_id}")
        
        if viz_data:
            logger.info(f"Returning visualization data with {len(viz_data.get('annotated_pages', []))} pages")
            return {"success": True, "data": viz_data}
        else:
            logger.warning(f"No visualization data found for {doc_id} in {db_id}")
            return {"success": False, "message": "No visualization data found"}
    except Exception as e:
        logger.error(f"Failed to get visualization data, {e}, {db_id=}, {doc_id=}, {traceback.format_exc()}")
...
@knowledge.put("/databases/{db_id}/documents/{doc_id}/filename")
async def update_document_filename(
    db_id: str, doc_id: str, new_filename: str = Body(..., embed=True), current_user: User = Depends(get_admin_user)
):
    """更新文档文件名"""
    logger.debug(f"Update document {doc_id} filename in {db_id} to {new_filename}")
    try:
        if not new_filename or not new_filename.strip():
            raise HTTPException(status_code=400, detail="文件名不能为空")
        
        result = await knowledge_base.update_file_name(db_id, doc_id, new_filename.strip())
        return {"message": "文件名更新成功", "file_info": result}
    except Exception as e:
        logger.error(f"更新文件名失败 {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=400, detail=f"更新文件名失败: {e}")


@knowledge.put("/databases/{db_id}/documents/{doc_id}/chunks/{chunk_id}")
async def update_chunk_content(
    ...
    """更新chunk内容"""
    logger.debug(f"Update chunk {chunk_id} content in document {doc_id} of database {db_id}")
    try:
        if not new_content or not new_content.strip():
            raise HTTPException(status_code=400, detail="chunk内容不能为空")
        
        result = await knowledge_base.update_chunk_content(db_id, doc_id, chunk_id, new_content.strip())
        return {"message": "chunk内容更新成功", "chunk_info": result}
    except NotImplementedError as e:
        logger.error(f"更新chunk内容失败（功能未实现）: {e}")
        raise HTTPException(status_code=501, detail=str(e))
    except Exception as e:
        logger.error(f"更新chunk内容失败 {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=400, detail=f"更新chunk内容失败: {e}")
